        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '汉服名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.hanfuName" placeholder="汉服名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('hanfuCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('hanfuCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('hanfuCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('hanfuCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gaoxiaohanfuzulinwangzhan/upload/hanfuCollectionMuBan.xls"
                        >批量导入汉服收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('hanfuCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="hanfuCollectionUploadSuccess"
                                :on-error="hanfuCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('hanfuCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入汉服收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('hanfuCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "hanfuCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('hanfuCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="hanfuCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.hanfuCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('hanfuCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('hanfuCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('hanfuCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'hanfu_collection',//当前表表名,
                        sumColum : 'hanfu_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'hanfu_collection_name',//分组字符串字段
                        // types : 'hanfu_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //     var statistic = this.$echarts.init(document.getElementById("statistic"),'macarons');
                //     let params = {
                //         tableName: "hanfu_collection",
                //         groupColumn: "hanfu_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['hanfuCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "hanfuCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            hanfuCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "hanfuCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            hanfuCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "hanfuCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            hanfuCollectionTypes:hanfuCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
